...
### 2026-09-23（v106 = 読む・記す操作と情景のつながり、実装・検証）

- PR #89マージ済みを確認。`codex/taskare-reading-scenes` で作業。
- 仕様: `apps/task-calendar/specs/task-calendar-v106.md`。背景の連続性、文字選択中のスワイプ抑止、記念日メモ、タイマー2種、ライト切替色。実ユーザーデータは使用しない。
- ヘッダー・カレンダーフィルターの不透明な帯を撤去し、アプリ全体に共通の読み取り面を配置。短いページでも背景が途中で濃く戻らない。
- 記念日メモは任意文字列として互換追加しtextContent描画。通知サーバー用データには含めない。追加タイマーは既存画像を共用し取得量は増加なし。
- v106/v105/v104・オフラインテスト成功。メモの再起動/編集/旧データ/HTML非解釈、入力・文字選択・複数指の抑止/通常スワイプ、タイマー2種完了記録、320/390/1440px明暗36組合せ、日/月・横画面を確認。実機iPhone選択ハンドル操作は未検証。
- コミット `105edae` 作成。pushは安全審査で拒否（今回のペイロードへの明示承認不足）。送信先 `lovesoccer4798-debug/html-apps`、ブランチ `codex/taskare-reading-scenes`。迂回せず今回の送信承認待ち。PR未作成、マージなし。
- その後ユーザーが今回の送信・PR作成を明示承認。push成功、[PR #90](https://github.com/lovesoccer4798-debug/html-apps/pull/90)作成済み。マージは行っていない。

### 2026-09-23（v105 = リマインド表示修正・情景デザイン、ローカル検証完了）

- PR #88のマージを確認。専用ブランチ `codex/taskare-immersive-designs` で作業。
- 仕様: `apps/task-calendar/specs/task-calendar-v105.md`。通知チェックの共通CSS衝突、モバイルヘッダー両端、名称変更、3デザイン・タイマー1種が対象。通知サーバー未設定の状況は変わらない。
- 共通inputのappearance:noneがチェック表示を消す問題を局所修正。6項目のタップ・保存・再表示・解除を検証。
- 旧3デザインのモバイルヘッダーを全幅化。「デザイン」へ改称。生成したローカル画像でアクアリウム・木漏れ日の森・銀河のアトラスを追加、明暗/OS切替に対応。アクアリウムタイマーは既存計時・完了記録を再利用。
- v105/v104/v102ブラウザテスト成功。320/390/1440pxの36組合せ、タイマー横画面、画像読み込み・日付の視認性を確認。オフラインテスト31資産、通知クライアント3件・Worker4件、構文・diffチェック成功。実機Safariは未確認。
- コミット `8341d32` 作成済み。pushは安全審査により拒否（送信先と内容の明示承認不足）。既存originは `https://github.com/lovesoccer4798-debug/html-apps.git` と確認。迂回せず承認待ち、PR未作成。承認後に専用ブランチをpushしてPR作成する。ユーザーデータ・本番通知設定は変更していない。
- その後、ユーザーが送信・PR作成を明示承認。push成功、[PR #89](https://github.com/lovesoccer4798-debug/html-apps/pull/89) 作成済み。マージは行っていない。

### 2026-09-23（v104 = 実装・ローカル検証完了、PR #88作成済み）

- 追加機能: 敬老の日等の祝日・6段階の通知日、Web Push登録/解除/テスト画面、あのねノートのリンク、無料の端末内語句ハイライト、素材付き3テーマ。
...
- 既存 `task-calendar-api` のビルド問題は変更対象外。mainとの差分取り込み不要をfetch後に確認。[PR #88](https://github.com/lovesoccer4798-debug/html-apps/pull/88) 作成済み、マージなし。Cloudflare初期設定の承認待ち。

### 2026-09-22（v104 = 大切な日の通知・ハイライト・リンク・テーマ、着手記録）

#### 2026-09-23 中断時の申し送り（以下は当時の記録、最新状況は上記）

- `app.js`/`index.html`/`sw.js` に途中の接続あり。新規 `reminder-core.js`（日付計算）、`reminder-client.js`（端末登録UI）、`reminder-worker/`（専用配信Worker、依存取得済み）、`record-tools.js`（リンク・語句抽出）、`expressive-themes.css` を追加。構文確認はapp.jsのみ実施。統合テスト・実機確認は未実施。
- 最優先: `sw.js` が参照する `assets/gallery.png` が未配置。生成済み元画像は `/Users/apple/.codex/generated_images/01a09a84-b0ff-7b60-8052-1a3d64723f05/exec-893b476f-c059-4f97-bbd5-214fb0cb9e96.png`。確認してコピーする。コラージュ画像は配置済み。
- 既存キャッシュ/版表示はv103のまま。新JS/CSSのみv104指定。検証後に版を統一する。現状では公開・マージ不可。
- 未完了: 祝日/年またぎ/うるう日テスト、通知Workerの認証・配信失敗/重複テスト、Playwright操作と明暗/モバイル/PCスクリーンショット、既存v102/origin回帰テスト。
- 通知のレビュー課題: 端末登録成功後に予定送信が失敗した際の中途半端な登録、複数端末の古い予定上書き、許可失効、リクエストサイズ上限、朝9時配信の無料枠CPU予算。利用者向け接続/解除/再送手順と、専用Worker初期設定の文書・ADRが未作成。既存task-calendar-apiには触れていない。
- ハイライトは生成AIではなく端末内の明示的な語句集計。あのねノートの既存文字列は維持。外部への日記送信なし。
- 新規PR/コミット/デプロイなし。サーバー未設定、スマホ閉鎖時の通知はまだ利用可能と案内しない。テスト完了後にCHANGELOG/README/STATUSを更新し、論理単位でコミット・PRを作成する。

- 仕様: `apps/task-calendar/specs/task-calendar-v104.md`。ブランチ: `codex/taskare-reminders-highlights`（PR #87マージ済みmain起点）
- ユーザーはアプリ終了中にも届く通知を希望。配信サーバーの初期設定が必要なWeb Pushを実装する。設定前の有効表示・未検証の配信成功は報告しない
- 振り返りは無料の端末内抽出。生成AIや感情診断として扱わず、根拠の記録を表示する
...
- 一覧は所属済みグループだけ表示。編集で所属変更、グループ画面で検索・複数選択して人を追加
- 所要時間なしの再生ボタン→10〜60分、またはカウントアップ。既存running保存を拡張し、一時停止・復元・完了記録に対応
- ナイトリフレクション差別化、新テーマ3種を追加。明暗・OS追従を確認
- Notionは今日を含む10日以内に限定し、1日の失敗で全体を止めない。予定名・時刻をメモへ追加。ノート本文の送信は既存コードで確認でき、本番未反映原因は未確定
- 本番データへの書き込みは行わず、架空データのPlaywrightテスト `apps/task-calendar/tests/v102.cjs` で検証。所属追加/解除・検索フォーカス・タイマー一時停止/復元/完了・10日境界/ノート本文/予定名/失敗再送・20通りのスマホPCテーマ明暗/OS追従がPASS。操作画面4枚も確認。`node --check` と `git diff --check` PASS。本番Notion疎通とiPhone実機は未検証
- 「大切なこと」のページは「原点ノート」を提案予定。実装せず、固定の言葉・今日の1枚・振り返りを壁打ちする
- PR: https://github.com/lovesoccer4798-debug/html-apps/pull/86 （`codex/taskare-profile-timer-themes`）。最新mainを取り込み済み。マージ・本番反映は行っていない
- 初回pushのチェック: GitHub CI 2件とCloudflare Pagesは成功、競合なし。`Workers Builds: task-calendar-api` は失敗（Workerコードは今回変更なし）。本番連携の成功は断言しない
...
- 通常の予定・タスク追加/編集シートに「月カレンダーで上に優先表示する」を追加。月ビューの並びは `ルーティンの上に固定` → `通常予定/タスクの優先表示` → `通常` の順にした
- ルーティン編集で新規追加した項目は、追加した日（期間はじめが未来なら期間はじめ）以降だけに反映。既存の行削除は従来どおり、過去がある場合は昨日でカットして未来だけ止める
- `APP_VERSION`、HTMLのクエリ、Service Workerキャッシュを v100 に更新

### 2026-09-15（v99 = PC全幅カレンダー・プロフィール帳整理）

- PC幅1100px以上で、左サイドバー固定＋カレンダー全幅のレイアウトへ追加上書き。スマホPWAの480px基準・下タブは維持
- プロフィール帳一覧に、お気に入り（星）・自由グループフォルダ・検索・あかさたな絞り込みを追加。整理メタ情報は `peopleProfiles[名前]._meta` と `settings.peopleGroups` に保存し、既存のプロフィール本文と分離
- `APP_VERSION`、HTMLのクエリ、Service Workerキャッシュを v99 に更新

### 2026-09-13（v98 = ギャラリー系テーマ5種・PC向けレイアウト）

- 設定 → テーマ（配色）に、添付画像の方向性をCSSで再現した「スカイグラス」「ナイトリフレクション」「ボタニカル」「サンセットグラス」「ミニマルアート」を追加。各テーマは `data-theme="light|dark"` とOS追従の両方で色・背景レイヤーが切り替わる
- PC幅（920px以上）では同じURLのまま左側ナビ＋広いカレンダーへ切り替え。スマホPWAは既存の下タブ/480px基準を維持し、PCの月ビュー・予定表ビューだけセルを広く使えるようにした
- `APP_VERSION`、HTMLのクエリ、Service Workerキャッシュを v98 に更新

### 2026-09-13（v97 = タイマー3種追加・プロフィール帳一覧・Notion未反映日同期）

- タイマー（フォーカス）デザインに「夜空」「温泉」「森」を追加。`TIMER_STYLES` / `TIMER_ART` / `#focus[data-timer]` CSS / 横向き縮小 / focus-link色 / theme-color を追加し、APP_VERSION・Service Workerキャッシュを v97 に更新
...
### 2026-09-13（v1.69.0 / アプリ内 v96 = Eclipseテーマトグル）

- 画面上部のテーマボタンを、ライトでは太陽、ダークでは星空と三日月になる横長のEclipseトグルへ変更
- 切り替え時は影が横切る小さな日食アニメーションを表示し、`prefers-reduced-motion` では動きを止める
- 設定画面の「自動／ライト／ダーク」は維持。上部トグルは現在の実効テーマを基準にライト／ダークを直接往復する
