...
3. **検収質問を5つ投げる**（「アップロードした」ではなく「正しく答えた」が完了 — 質問検収）:
   - 「NESTの六箇条を教えて」（憲法が読めているか）
   - 「はじめてモードとは？」（Handbookの章が読めているか）
   - 「Handbookとはどんな場所？」（「毎日開くホーム・生きた地図」と答えれば、役割定義が最新）
   - 「Creator Studioは、いつ・何のために使う？」（アプリの使いどころが答えられるか）
   - 「**いちばん最近完了したPhaseは？**」（← 最新情報が入ったかの決め手）
4. 5問すべてが正本と一致したら完了。[Handbook表紙の品質パネル](README.md)の「NotebookLM同期」行に日付を記録する（鮮度情報の正本はあのパネル — [ADR](adr/20260714-handbook-living-map.md)）。1問でも誤答なら、該当ソースを入れ替えて再検収

### 完了条件

検収質問5問への正答＋Handbook品質パネルへの日付記録。**「アップロードした」は完了ではない**（正しく答えて初めて同期＝質問検収完了）。あわせて品質パネルの他の行（この地図の最終更新など）が古びていないかも一目確認する（月1点検を兼ねる）。
