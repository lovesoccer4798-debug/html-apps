...
4. 4問すべてが正本と一致したら完了。[Handbook表紙の品質パネル](README.md)の「NotebookLM同期」行に日付を記録する（鮮度情報の正本はあのパネル — [ADR](adr/20260714-handbook-living-map.md)）

### 完了条件

検収質問4問への正答＋Handbook品質パネルへの日付記録。**「アップロードした」は完了ではない**（正しく答えて初めて同期）。あわせて品質パネルの他の行（この地図の最終更新など）が古びていないかも一目確認する（月1点検を兼ねる）。
