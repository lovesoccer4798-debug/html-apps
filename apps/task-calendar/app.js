...
function monthPinRank(it) {
  if (!it.ref || !it.ref.pin) return 2;
  return it.ref.routineId ? 0 : 1; // ルーティン固定を最優先、単発の優先表示はその下
}
function sortMonthItems(a, b) {
  const ar = monthPinRank(a);
  const br = monthPinRank(b);
  if (ar !== br) return ar - br;
  if (ar === 1) return (a.ref.pinOrder || a.ref.createdAt || 0) - (b.ref.pinOrder || b.ref.createdAt || 0);
  return 0; // itemsForの時刻順・作成順を保つ
}
...
function renderMonth(body) {
  ...
  for (let i = 0; i < 42; i += 1) {
    ...
    items.sort(sortMonthItems); // 固定表示: ルーティン固定 → 単発の優先表示 → 通常（通常内は時刻順）
...
function renderPeopleBook() {
  ...
  searchInput.addEventListener('input', () => {
    ...
    const clear = search.querySelector('.peoplebook-clear');
    if (clear) clear.hidden = !ui.peopleQuery;
    renderPeopleBookResults(names, groups);
  });
  search.append(searchInput);
  const clear = el('button', 'peoplebook-clear');
  clear.type = 'button';
  clear.hidden = !ui.peopleQuery;
  ...
  const results = el('div', 'peoplebook-results');
  results.id = 'peoplebook-results';
  body.append(results);
  renderPeopleBookResults(names, groups);
}
function renderPeopleBookResults(names = allPeopleNames(), groups = peopleGroups()) {
  const body = $('#peoplebook-body');
  if (!body) return;
  const results = $('#peoplebook-results') || body;
  results.textContent = '';
...
$('#opt-pin').addEventListener('click', () => setOpt('#opt-pin', !getOpt('#opt-pin')));
...
$('#sheet-form').addEventListener('submit', (e) => {
  ...
  const pinMonth = getOpt('#opt-pin'); // 「月」カレンダーで上に優先表示
  ...
  if (ui.editing) {
    ...
    if (pinMonth) {
      ui.editing.ref.pin = true;
      ui.editing.ref.pinOrder = ui.editing.ref.pinOrder || Date.now();
    } else {
      delete ui.editing.ref.pin;
      delete ui.editing.ref.pinOrder;
    }
...
function routineItemRow(data = {}) {
  ...
  // 予定・タスクごとの個別オプション（色 / 上に固定 / 月に出さない）
  const opts = el('div', 'r-item-opts');
  const colorWrap = el('div', 'r-item-colors');
  let curColor = data.color || ''; // '' = ルーティンの色を継承
  const swatchEls = [];
  ['', ...Object.keys(ACCENTS)].forEach((id) => {
    const sw = el('button', `r-csw${id === '' ? ' is-none' : ''}${curColor === id ? ' is-active' : ''}`);
    sw.type = 'button'; sw.dataset.color = id;
    if (id) sw.style.background = effectiveDark() ? ACCENTS[id].dark : ACCENTS[id].light;
    sw.setAttribute('aria-label', id ? ACCENTS[id].name : 'ルーティンの色を継承');
    if (id === '') sw.textContent = '継';
    sw.addEventListener('click', () => { curColor = id; swatchEls.forEach((b) => b.classList.toggle('is-active', b.dataset.color === id)); });
    colorWrap.append(sw); swatchEls.push(sw);
  });
  const pin = el('button', `r-opt-chip${data.pin ? ' is-on' : ''}`);
  pin.type = 'button'; pin.innerHTML = `${ICONS.pin}<span>上に固定</span>`;
  pin.setAttribute('aria-label', '月カレンダーで日付の真下に優先表示');
  pin.addEventListener('click', () => pin.classList.toggle('is-on'));
  const hide = el('button', `r-opt-chip${data.hideMonth ? ' is-on' : ''}`);
  hide.type = 'button'; hide.textContent = '月に出さない';
  hide.setAttribute('aria-label', '月カレンダーにこの項目を表示しない');
  ...
  row._fields = {
    title, rep, time, min,
    getWeekdays: () => wdBtns.filter((b) => b.classList.contains('is-on')).map((b) => Number(b.dataset.dow)),
...
$('#r-form').addEventListener('submit', (e) => {
  ...
  for (const row of $('#r-items').children) {
    ...
    const itemColorSel = f.getColor(); // 予定・タスクごとの色（null＝ルーティン色を継承）
    const itemPin = f.getPin(); // 月カレンダーで日付の真下に固定
    const itemHide = f.getHideMonth(); // 月カレンダーに出さない
    ...
    if (existing) {
      ...
      const itemStart = r.periodStart && r.periodStart > today ? r.periodStart : today;
      const base = { id: newId(rtype === 'event' ? 'e' : 't'), routineId: r.id, title: t2, time, repeat, weekdays: weekdays || undefined, startDate: itemStart, exDates: [], color: itemColorSel || undefined, pin: itemPin || undefined, hideMonth: itemHide || undefined, link: f.getLink() || undefined, createdAt: Date.now() };
