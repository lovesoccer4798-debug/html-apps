...
- **作成日**: 2026-07-15
- **最終更新**: 2026-07-15
- **ステータス**: 下書き（オーナー承認＋Firebaseプロジェクト準備後に実装）
...
- 共有カレンダーの項目は `sharedCalendars/{calId}/items` に置き、参加者全員が読み書き（メンバーは双方編集可）
- チップに参加カレンダーが並ぶ（自分のと同じ操作感）。退出・オーナーによるメンバー解除あり
- Firestoreセキュリティルール: 本人のuid配下のみ／共有はメンバーリストに載るuidのみ読み書き可
...
1. ログインは**Googleのみ**でよいか？（Apple/匿名は後回し推奨）
2. Phase A→Bの2段階でよいか？（一気にBまで欲しい？）
3. 共有メンバーの権限は**全員編集可**（TimeTree同様）でよいか？閲覧のみモードも要る？
