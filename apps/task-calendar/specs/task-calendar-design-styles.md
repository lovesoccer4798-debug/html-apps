...
v98ではPCでもまだ「スマホ幅のアプリが中央に残る」見え方が強かった。v99では `@media (min-width: 1100px)` を追加し、Notion Calendar / Google Calendar のように画面全体を予定表として使う。

- スマホ: 既存の 480px 基準・下タブを維持
- 920px以上: 既存の軽いPC化（左ナビ・広めセル）
- 1100px以上: 左サイドバーを 248px 固定にし、`.app` の最大幅を外してカレンダーを全幅化
- 月ビュー/予定表/TimeTree風のセルは角丸カードではなく、罫線でつながるカレンダー面に寄せる
- DOMは共通のまま。機能分岐はCSS media queryで行うため、スマホPWAの操作には影響させない

## v100: 月カレンダーの優先表示

通常の予定・タスクの追加/編集シートに「月カレンダーで上に優先表示する」を追加する。

- 並び順は `ルーティンの上に固定` → `通常予定/タスクの優先表示` → `通常` の順
- 優先表示ONの通常予定/タスク同士は、ONにした順（`pinOrder`、なければ作成順）で並ぶ
- 予定・タスク本体に `pin: true` / `pinOrder` を保存し、月ビューの表示順だけに使う
- スマホ/PCどちらも同じデータを使い、レイアウト分岐は既存のレスポンシブCSSに任せる
