...
- Handbook実証フェーズの枠組み: 実証開始チェックリスト（時系列10項目）と実証終了条件6つ（実運用7日以上・Creator Studio5回以上・NotebookLM質問検収5問正答・初見ユーザーテスト合格・must指摘ゼロ・**チャット非依存で運用できた**）を先行定義（`tasks/in-progress/20260714-handbook-validation.md`）。NotebookLM検収は「同期完了」でなく「質問検収完了」を完了条件に（検収質問を5問化 — ai-tools.md）。実証ログ（1日1〜3分・日付/使った機能/良かったこと/迷ったこと/must指摘の5項目）を追加 — 7日後の正式採用レビューで具体的な改善点を残すため
